...
import { mockTeacherCourses, mockTeacherSchedule, PERIODS, DAYS } from "@/data/mock";
...
export default function SchedulePage() {
  // Today's classes
  const todaySlots = (mockTeacherSchedule[todayCapitalized] ?? []).filter(Boolean);

  // Weekly totals
  const totalPeriods = Object.values(mockTeacherSchedule)
    .flatMap((day) => day)
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: "Secciones a cargo", value: mockTeacherCourses.length, icon: Users, bg: "bg-amber-50", text: "text-amber-600" },
        ].map((s) => (
      ...
      <Card className="border-none shadow-sm rounded-xl overflow-hidden">
        <CardContent className="p-0">
          ...
          {PERIODS.map((period, pi) => (
            <>
              {pi === 3 && (
                <div key="recreo" className="grid grid-cols-[110px_repeat(5,1fr)] bg-amber-50 border-y border-amber-200">
              ...
              <div
                key={period}
                className="grid grid-cols-[110px_repeat(5,1fr)] border-b border-gray-100 hover:bg-gray-50/40 transition-colors"
              >
                {/* Time */}
                ...
                {/* Cells */}
                {DAYS.map((day) => {
                  const slot = mockTeacherSchedule[day]?.[pi] ?? null;
                  const isToday = day === todayCapitalized;
                  const style = slot
