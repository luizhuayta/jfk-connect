...
import { CheckCircle2, XCircle, Clock, Save, CalendarDays, Users } from "lucide-react";
import {
  mockTeacherCourses,
  mockCourseStudents,
  mockClassSessions,
...
export default function AttendancePage() {
  const [activeCourseId, setActiveCourseId] = useState("tc-1");
  ...
  // Load existing session or default all to "A"
  const existingSession = mockClassSessions.find(
  ...
  const [records, setRecords] = useState<Record<string, Status>>(() =>
    Object.fromEntries(
      students.map((s) => [
        s.id,
        (existingSession?.records.find((r) => r.studentId === s.id)?.status ?? "A") as Status,
      ])
    )
  );

  function switchContext(courseId: string, date: string) {
    const session = mockClassSessions.find(
      (s) => s.courseId === courseId && s.date === date
    );
    const sts = mockCourseStudents[courseId] ?? [];
    setRecords(
      Object.fromEntries(
        sts.map((s) => [
          s.id,
          (session?.records.find((r) => r.studentId === s.id)?.status ?? "A") as Status,
        ])
      )
    );
    setSaved(false);
  ...
  function handleCourse(id: string) {
    ...
    switchContext(id, activeDate);
  }

  function handleDate(d: string) {
    setActiveDate(d);
    switchContext(activeCourseId, d);
  }

  function toggle(studentId: string) {
    setRecords((prev) => {
      const cur = prev[studentId];
      const next: Status = cur === "A" ? "T" : cur === "T" ? "F" : "A";
  ...
  const history = RECENT_DATES.filter((d) => d !== activeDate)
    .map((date) => {
    ...
    .filter(Boolean)
    .slice(0, 5) as { date: string; A: number; F: number; T: number; pct: number }[];
                      ...
                      <TableCell className="text-center">
                        <div className="flex items-center justify-center gap-1">
                          {(["A", "T", "F"] as Status[]).map((s) => (
                            <button
                              key={s}
                              onClick={() => setRecords((prev) => ({ ...prev, [student.id]: s }))}
