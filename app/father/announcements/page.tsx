...
import { Bell, AlertTriangle, Info, Megaphone, ChevronDown, ChevronUp } from "lucide-react";
...
export default function AnnouncementsPage() {
  ...
  const [readIds, setReadIds] = useState<Set<string>>(
    new Set(mockAnnouncements.filter((a) => a.read).map((a) => a.id))
  );
  ...
  const unreadCount = mockAnnouncements.filter((a) => !readIds.has(a.id)).length;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {(["urgente", "importante", "general", "informativo"] as AnnouncementCategory[]).map((cat) => {
          ...
          const count = mockAnnouncements.filter((a) => a.category === cat).length;
          const Icon = cfg.icon;
          return (
            <button
              key={cat}
