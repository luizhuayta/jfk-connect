...
import { mockStudents, mockSchedules, DAYS, PERIODS } from "@/data/mock";
...
export default function SchedulePage() {
  ...
  const schedule = mockSchedules[activeStudentId] ?? {};

  // Build unique subjects for the legend
  const subjects = Array.from(
    new Set(Object.values(schedule).flat().map((p) => p.subject))
  ).sort();
      ...
      <Card className="border-none shadow-sm rounded-xl overflow-hidden">
        <CardContent className="p-0">
          ...
          {PERIODS.map((period, pi) => (
            <>
              {pi === 3 && (
                <div key="recreo" className="grid grid-cols-[120px_repeat(5,1fr)] bg-amber-50 border-y border-amber-200">
              ...
              <div
                key={period}
                className="grid grid-cols-[120px_repeat(5,1fr)] border-b border-gray-100 hover:bg-gray-50/50 transition-colors"
              >
                {/* Time */}
